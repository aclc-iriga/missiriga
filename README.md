...
3. Clone or download this repository to your XAMPP **htdocs** folder.
   The final path should be `path_to/xampp/htdocs/miss-iriga`.
...
5. Inside [phpMyAdmin](http://localhost/phpmyadmin),
   create a MySQL database named `miss-iriga` and import [miss-iriga.sql](miss-iriga.sql) into it.

6. Open the terminal and navigate to the project directory **miss-iriga**.
...
9. Open your web browser and access <http://localhost:5199/miss-iriga/> to view the application.
...
2. Inside [phpMyAdmin](http://localhost/phpmyadmin),
   create a MySQL database named `test-miss-iriga` and import [miss-iriga.sql](miss-iriga.sql) into it.
...
2. Access the application by visiting `http://[host_name]/miss-iriga`,
   where `host_name` is the **IP address** or **host name** of the server in the network.
   For example:
     - <http://localhost/miss-iriga>
     - <http://192.168.1.99/miss-iriga>


#### Admin Dashboard
Access the **admin dashboard** at <http://localhost/miss-iriga/app>.
